...
Public Class RekapPiutang
    ...
    Sub excel()
        ...
        Try
            ...
            For i = 0 To dgvRanap.RowCount - 1
                For j = 0 To dgvRanap.ColumnCount - 2
                    For k = 1 To dgvRanap.ColumnCount - 1
                        xlWorkSheet.Cells(1, k) = dgvRanap.Columns(k - 1).HeaderText
                        xlWorkSheet.Cells(i + 2, j + 1) = dgvRanap(j, i).Value.ToString()
                    Next
                Next
            Next
            ...
            MessageBox.Show("Hasil export tersimpan di folder " & IO.Path.GetDirectoryName(SaveFileDialog1.FileName))
    ...
    Sub DaftarAsalPx(norm As String, mrs As Date)
        ...
        query = "SELECT * FROM t_eklaimjprajal
                  ...
                  UNION ALL
                 SELECT * FROM t_eklaimjprajalumum 
                  WHERE noRM = '" & norm & "' AND (SUBSTR(tglMasuk,1,10) BETWEEN '" & Format(DateAdd(DateInterval.Day, -1, mrs), "yyyy-MM-dd") & "' AND '" & Format(DateAdd(DateInterval.Day, 1, mrs), "yyyy-MM-dd") & "')"
    ...
    Sub simpanTotalHarianRajal()
        ...
        strRkp = "INSERT INTO t_rekaptotalharianrajal(tglRekap,admin,jasaVisitKonsul,prosedurbedah,
                                                      endoscopy,bronkoscopy,hd,cvc,ivp,
    ...
    Sub updateTotalHarianRanap()
        ...
        strRkp = "UPDATE t_rekaptotalharianranap SET akomodasiAdmin = @akomodasiAdmin, akomodasiRuang = @akomodasiRuang, jasaVisitKonsul = @jasaVisitKonsul, prosedurbedah = @prosedurbedah,
    ...
    Sub updateTotalHarianRajal()
        ...
        strRkp = "UPDATE t_rekaptotalharianrajal SET admin = @admin,jasaVisitKonsul = @jasaVisitKonsul,
    ...
    Private Sub RekapPiutang_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        btnPiutang.BackColor = Color.FromArgb(209, 232, 223)
        ...
        With txtFilter
            .Items.Add("Rawat Jalan")
            .Items.Add("Rawat Inap")
        End With
...
#End Region
    ...
    Private Sub dgvDokterAll_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs)
        'For i As Integer = 14 To dgvDokterAll.Columns.Count - 1
        '    dgvDokterAll.Columns(i).DefaultCellStyle.Format = "N0"
        'Next

        'dgvDokterAll.DefaultCellStyle.Font = New Font("Segoe UI", 9, FontStyle.Regular)
        'dgvDokterAll.DefaultCellStyle.SelectionBackColor = Color.PaleTurquoise
        'dgvDokterAll.DefaultCellStyle.SelectionForeColor = Color.Black
        'dgvDokterAll.DefaultCellStyle.WrapMode = DataGridViewTriState.True

        'For i As Integer = 0 To dgvDokterAll.Rows.Count - 1
        '    If i Mod 2 = 0 Then
        '        dgvDokterAll.Rows(i).DefaultCellStyle.BackColor = Color.White
        '    Else
        '        dgvDokterAll.Rows(i).DefaultCellStyle.BackColor = Color.WhiteSmoke
        '    End If
        'Next
    End Sub
    ...
    Private Sub dgvDokterAll_RowPostPaint(sender As Object, e As DataGridViewRowPostPaintEventArgs)
        Dim dg As DataGridView = DirectCast(sender, DataGridView)
        Dim rowNumber As String = (e.RowIndex + 1).ToString()
...
#Region "Dokter Per Pasien"
        Dim totJVisit As Double
        For i As Integer = 0 To DataGridView1.RowCount - 1
            totJVisit += DataGridView1.Rows(i).Cells(5).Value
        Next

        DataGridView1.Rows.Add("TOTAL", Nothing, Nothing, Nothing, Nothing, totJVisit)

        lastIndexPerpx = DataGridView1.Rows.Count - 1
#End Region
    End Sub
    ...
    Private Sub dgvRajal_DataBindingComplete(sender As Object, e As DataGridViewBindingCompleteEventArgs) Handles dgvRajal.DataBindingComplete
        dgvRajal.Rows(lastIndexRj).DefaultCellStyle.Font = New Font("Segoe UI", 9, FontStyle.Regular)
        dgvRajal.Rows(lastIndexRj).DefaultCellStyle.BackColor = Color.FromArgb(26, 141, 95)
        dgvRajal.Rows(lastIndexRj).DefaultCellStyle.ForeColor = Color.FromArgb(232, 243, 239)
    End Sub

    Private Sub dgvRanap_DataBindingComplete(sender As Object, e As DataGridViewBindingCompleteEventArgs) Handles dgvRanap.DataBindingComplete
        dgvRanap.Rows(lastIndexRi).DefaultCellStyle.Font = New Font("Segoe UI", 9, FontStyle.Regular)
        dgvRanap.Rows(lastIndexRi).DefaultCellStyle.BackColor = Color.FromArgb(26, 141, 95)
        dgvRanap.Rows(lastIndexRi).DefaultCellStyle.ForeColor = Color.FromArgb(232, 243, 239)
    End Sub

    Private Sub dgvDokterAll_DataBindingComplete(sender As Object, e As DataGridViewBindingCompleteEventArgs)
        'dgvDokterAll.Rows(lastIndexDok).DefaultCellStyle.Font = New Font("Segoe UI", 9, FontStyle.Regular)
        'dgvDokterAll.Rows(lastIndexDok).DefaultCellStyle.BackColor = Color.FromArgb(26, 141, 95)
        'dgvDokterAll.Rows(lastIndexDok).DefaultCellStyle.ForeColor = Color.FromArgb(232, 243, 239)
    End Sub

    Private Sub DataGridView1_DataBindingComplete(sender As Object, e As DataGridViewBindingCompleteEventArgs) Handles DataGridView1.DataBindingComplete
        DataGridView1.Rows(lastIndexPerpx).DefaultCellStyle.Font = New Font("Segoe UI", 9, FontStyle.Regular)
        DataGridView1.Rows(lastIndexPerpx).DefaultCellStyle.BackColor = Color.FromArgb(26, 141, 95)
        DataGridView1.Rows(lastIndexPerpx).DefaultCellStyle.ForeColor = Color.FromArgb(232, 243, 239)
    End Sub
End Class